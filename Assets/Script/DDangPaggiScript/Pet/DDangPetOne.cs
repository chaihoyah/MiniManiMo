...
public class DDangPetOne : MonoBehaviour {
    ...
    IEnumerator PetMove()
    {
        while(true)
        {
            if (Vector3.Distance(Panda.transform.position, this.transform.position) > 0.25f)
            {
                MoveVec = Panda.transform.position - this.transform.position;
                MoveVec.Normalize();
                this.transform.position += MoveVec * 0.015f;
            }
