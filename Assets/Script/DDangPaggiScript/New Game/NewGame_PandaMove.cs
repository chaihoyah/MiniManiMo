using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.UI;

public class NewGame_PandaMove : MonoBehaviour
{

    public float moveSpeed;
    public VirtualJoystick_NewGame MoveJoystick;

    float angle;
    float speed;
    int i;
    Quaternion di;
    Rigidbody Rig;

    // Use this for initialization
    void Start()
    {
        speed = 0.5f;
        Rig = this.GetComponent<Rigidbody>();
        if (PlayerPrefs.GetInt("SwimShoes2").Equals(2))
            speed *= 1.2f;
        else if (PlayerPrefs.GetInt("SwimShoes3").Equals(2))
            speed *= 1.4f;
        else
            speed *= 1f;
        StartCoroutine(StartMove());
        StartCoroutine(StartGG());
    }

    // Update is called once per frame
    IEnumerator StartMove()
    {

        while (true)
        {
            if (Time.timeScale.Equals(1))
            {
                angle = MoveJoystick.angle;
                if (angle != 0)
                {//&& camRotate.RotateBackOn.Equals(false))
                    transform.rotation = Quaternion.Euler(-90, angle * (180 / Mathf.PI) - 180, 0);
                }
                if (MoveJoystick.isDrag)
                {
                    Vector3 MoveDir = new Vector3(0, -speed, 0);
                    transform.Translate(MoveDir);
                }

                yield return new WaitForEndOfFrame();
            }
            else
                yield return new WaitForSeconds(0.01f);
        }
    }

    IEnumerator StartGG()
    {
        while (true)
        {
            Rig.velocity = Vector3.zero;
            Rig.velocity = Vector3.zero;
            yield return new WaitForSeconds(1f);
        }
    }


    float angleChk(float ang)
    {
        if (ang >= -2.625 && ang <= -1.875) return -2.25f;
        else if (ang > -1.875 && ang <= -1.125) return -1.5f;
        else if (ang > -1.125 && ang <= -0.375) return -0.75f;
        else if (ang > -0.375 && ang <= 0.375) return 0;
        else if (ang > 0.375 && ang <= 1.125) return 0.75f;
        else if (ang > 1.125 && ang <= 1.875) return 1.5f;
        else if (ang > 1.875 && ang <= 2.625) return 2.25f;
        else
        {
            return ang = -3f;
        }
    }
}