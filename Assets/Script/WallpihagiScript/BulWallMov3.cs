...
public class BulWallMov3 : MonoBehaviour
{
    ...
    void Update()
    {
        if (!WallCount.isFinished)
        {
            ...
            if (this.transform.position.z >= 2.8)
            {
                ...
                if (this.gameObject.transform.GetComponent<RandomPizzaScore>().isRandom) WallCount.Score += 5 * PlayerCollids.BullWallPlusScore;
                else WallCount.Score += PlayerCollids.BullWallPlusScore;
                Wall.objPoolArr[3].PushObject(this.gameObject);
                WallCount.Wallcnt++;
            }
        }
    }
